...
export function Chatbot() {
  ...
  // Core API endpoints (preserved from original)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!jobId) {
        throw new Error('Submission failed ☹︎');
      }
      ...
      while (!done && tries < 60) {
        ...
        if (json.status === 'done') {
          ...
          setAnswer(json.brief || JSON.stringify(json, null, 2));
          break;
        }

        setStatus(`Analyzing your idea... (${++tries * 10}s)`);
            ...
            <ScrollArea className="h-[360px] w-full overflow-auto p-1">
              <div className="px-6">
                ...
                <p className="md:text-md lg:text-md mx-auto mt-1 text-center text-xs text-primary/60 md:max-w-2xl">
                  Describe your startup idea and get AI-powered evaluation
                </p>
              </div>
